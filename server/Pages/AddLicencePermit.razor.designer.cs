...
{
    public partial class AddLicencePermitComponent : ComponentBase
    {
        ...
        {
            licencepermit = new Clear.Risk.Models.ClearConnection.LicencePermit(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.LicencePermit args)
        {
            ...
            try
            {
                var clearConnectionCreateLicencePermitResult = await ClearConnection.CreateLicencePermit(licencepermit);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(licencepermit);
            }
            catch (System.Exception clearConnectionCreateLicencePermitException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new LicencePermit!");
                IsLoading = false;
                StateHasChanged();
