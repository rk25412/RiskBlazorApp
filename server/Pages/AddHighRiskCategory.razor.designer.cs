...
{
    public partial class AddHighRiskCategoryComponent : ComponentBase
    {
        ...
        {
            highriskcategory = new Clear.Risk.Models.ClearConnection.HighRiskCategory(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.HighRiskCategory args)
        {
            ...
            try
            {
                var clearConnectionCreateHighRiskCategoryResult = await ClearConnection.CreateHighRiskCategory(highriskcategory);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(highriskcategory);
            }
            catch (System.Exception clearConnectionCreateHighRiskCategoryException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new HighRiskCategory!");
                IsLoading = false;
                StateHasChanged();
