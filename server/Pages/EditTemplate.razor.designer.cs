...
{
    public partial class EditTemplateComponent : ComponentBase
    {
        ...
        {
            var clearConnectionGetTemplateByIdResult = await ClearConnection.GetTemplateById(ID);
            template = clearConnectionGetTemplateByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.Template args)
        {
            try
            {
                var clearConnectionUpdateTemplateResult = await ClearConnection.UpdateTemplate(ID, template);
                DialogService.Close(template);
            }
            catch (System.Exception clearConnectionUpdateTemplateException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update Template");
            }
        }
