...
{
    public partial class EditSurveyQuestion: ComponentBase
    {
        ...
        {
            if(surveyquestion.QUESTION_TYPE_ID == 2 || surveyquestion.QUESTION_TYPE_ID == 4)
            {
                if(getSurveyAnswersResult.Count() == 1)
                {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Atleast one answer is required",1800000);
                    return;
                }
            }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.SurveyQuestion args)
        {
            ...
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update SurveyQuestion",1800000);
            }
        }
