...
{
    public partial class AddHazardValueComponent : ComponentBase
    {
        ...
        {
            hazardvalue = new Clear.Risk.Models.ClearConnection.HazardValue(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.HazardValue args)
        {
            try
            {
                var clearConnectionCreateHazardValueResult = await ClearConnection.CreateHazardValue(hazardvalue);
                DialogService.Close(hazardvalue);
            }
            catch (System.Exception clearConnectionCreateHazardValueException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new HazardValue!");
            }
        }
