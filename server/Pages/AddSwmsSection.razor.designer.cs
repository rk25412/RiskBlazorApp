...
{
    public partial class AddSwmsSectionComponent : ComponentBase
    {
        ...
        {
            swmssection = new Clear.Risk.Models.ClearConnection.SwmsSection(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.SwmsSection args)
        {
            ...
            try
            {
                var clearConnectionCreateSwmsSectionResult = await ClearConnection.CreateSwmsSection(swmssection);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(swmssection);
            }
            catch (System.Exception clearConnectionCreateSwmsSectionException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new SwmsSection!");
                IsLoading = false;
                StateHasChanged();
            }
        }
