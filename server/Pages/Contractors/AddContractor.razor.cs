...
{
    public partial class AddContractor: ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var company = await ClearConnection.GetPersonByPersonId(Security.getUserId());

            person = new Clear.Risk.Models.ClearConnection.Person()
            {

                COMPANYTYPE = 5,
                PARENT_PERSON_ID = Security.getUserId(),
                CURRENCY_ID = company.CURRENCY_ID,
                PERSONAL_COUNTRY_ID = company.PERSONAL_COUNTRY_ID,
                PERSONAL_STATE_ID = company.PERSONAL_STATE_ID,
                BUSINESS_COUNTRY_ID = company.BUSINESS_COUNTRY_ID,
                BUSINESS_STATE_ID = company.BUSINESS_STATE_ID,
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.Person args)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    ...
                    UriHelper.NavigateTo("edit-contractor" + "/" + args.PERSON_ID.ToString());
                }
                else
                {
