...
{
    public partial class AddConsequenceComponent : ComponentBase
    {
        ...
        {
            consequence = new Clear.Risk.Models.ClearConnection.Consequence(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.Consequence args)
        {
            ...
            try
            {
                var clearConnectionCreateConsequenceResult = await ClearConnection.CreateConsequence(consequence);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(consequence);
            }
            catch (System.Exception clearConnectionCreateConsequenceException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new Consequence!");
                IsLoading = false;
                StateHasChanged();
            }
        }
