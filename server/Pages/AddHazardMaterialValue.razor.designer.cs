...
{
    public partial class AddHazardMaterialValueComponent : ComponentBase
    {
        ...
        {
            hazardmaterialvalue = new Clear.Risk.Models.ClearConnection.HazardMaterialValue(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.HazardMaterialValue args)
        {
            ...
            try
            {
                var clearConnectionCreateHazardMaterialValueResult = await ClearConnection.CreateHazardMaterialValue(hazardmaterialvalue);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(hazardmaterialvalue);
            }
            catch (System.Exception clearConnectionCreateHazardMaterialValueException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new HazardMaterialValue!");
                IsLoading = false;
                StateHasChanged();
            }
        }
