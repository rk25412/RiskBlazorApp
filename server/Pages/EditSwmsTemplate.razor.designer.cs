...
{
    public partial class EditSwmsTemplateComponent : ComponentBase
    {
        ...
        {
            var clearConnectionGetSwmsTemplateBySwmsidResult = await ClearConnection.GetSwmsTemplateBySwmsid(SWMSID);
            ...
            OnSectionChange(swmstemplate.TEMPLATEQUESTION);
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.SwmsTemplate args)
        {
            try
            {
                var clearConnectionUpdateSwmsTemplateResult = await ClearConnection.UpdateSwmsTemplate(SWMSID, swmstemplate);
                DialogService.Close(swmstemplate);
            }
            catch (System.Exception clearConnectionUpdateSwmsTemplateException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update SwmsTemplate");
            }
        }
