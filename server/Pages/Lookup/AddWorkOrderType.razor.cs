...
{
    public partial class AddWorkOrderType: ComponentBase
    {
        ...
        {
            workordertype = new Clear.Risk.Models.ClearConnection.WorkOrderType() { };
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Clear.Risk.Models.ClearConnection.WorkOrderType args)
        {
            ...
            try
            {
                var clearRiskCreateWorkOrderTypeResult = await ClearRisk.CreateWorkOrderType(workordertype);
                IsLoading = false;
                StateHasChanged();
                DialogService.Close(workordertype);
            }
            catch (System.Exception clearRiskCreateWorkOrderTypeException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new WorkOrderType!");
                IsLoading = false;
                StateHasChanged();
