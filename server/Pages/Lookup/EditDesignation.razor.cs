...
{
    public partial class EditDesignation: ComponentBase
    {
        ...
        {
            var clearRiskGetDesignationIdResult = await ClearRisk.GetDesigationId(int.Parse($"{DESIGNATION_ID}"));
            desigation = clearRiskGetDesignationIdResult;
        }
        protected async System.Threading.Tasks.Task Form0Submit(Desigation args)
        {
            ...
            try
            {
                var clearRiskUpdateDesignationResult = await ClearRisk.UpdateDesigation(int.Parse($"{DESIGNATION_ID}"), desigation);
                IsLoading = false;
                StateHasChanged();
