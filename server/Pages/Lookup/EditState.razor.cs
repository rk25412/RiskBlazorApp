...
{
    public partial class EditState: ComponentBase
    {
        ...
        {
            var clearRiskGetStateByIdResult = await ClearRisk.GetStateById(ID);
            state = clearRiskGetStateByIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(State args)
        {
            try
            {
                var clearRiskUpdateStateResult = await ClearRisk.UpdateState(ID, state);
                DialogService.Close(state);
            }
            catch (System.Exception clearRiskUpdateStateException)
            {
