...
{
    public class RunScheduleAssesment
    {
        ...
        private int AssesmentId { get; set; }

        public RunScheduleAssesment(ClearConnectionService context,
        ...
        public async Task Invoke(int Id, int companyId)
        {
            try
            {
                var assessments = await context.GetAssesments(new Query { Filter = $"i => i.ASSESMENTID == {Id}" });
                var assessment = assessments.FirstOrDefault();
                var person = await context.GetPersonByPersonId(companyId);
                if (person.CURRENT_BALANCE < person.Applicence.NETPRICE)
                {
                    if (assessment.ENTITY_STATUS_ID != 6)
                    {
                        assessment.AssessmentActivity = $"Assessment schedule stopped on {DateTime.Now} due to insufficient balance.";
                        assessment.ENTITY_STATUS_ID = 6;
                        var updateResult = context.UpdateAssesment(Id, assessment);
                    }
                    //RecurringJob.RemoveIfExists($"Assessment-{Id}");
                    await SendInsufficientBalanceEmail(person);
                    return;
                }

                if (assessment.ENTITY_STATUS_ID != 5)
                {
                    assessment.AssessmentActivity = $"Assessment schedule started on {DateTime.Now}.";
                    assessment.ENTITY_STATUS_ID = 5;
                    var updateResult = context.UpdateAssesment(Id, assessment);
                }

                AssesmentId = Id;

                var item = await context.CopyAssesmentByAssesmentid(AssesmentId);

                //context.GetAssesments(new Query() { Filter = $@"i => i.ISSCHEDULE == {true}" });
                //https://entityframeworkcore.com/knowledge-base/48873716/how-to-deep-clone-copy-in-ef-core

                var model = new Assesment();
                model.ASSESMENTID = 0;
                model.ASSESMENTDATE = DateTime.Now;
                model.CREATED_DATE = DateTime.Now;
                model.UPDATED_DATE = DateTime.Now;
                model.IS_DELETED = false;

                model.RISKASSESSMENTNO = GetFormNumber(item.RISKASSESSMENTNO);
                model.WORKSTARTDATE = DateTime.Now;
                model.WORKENDDATE = DateTime.Now;
                ...
                model.FRI = false;
                model.HOLIDAY = false;
                model.SCHEDULE_TYPE_ID = null;
                model.SCHEDULE_TIME = null;
                model.ParentAssessmentId = AssesmentId;
                ...
                {
                    docs.Add(new AssessmentDocument
                    {
                ...
                model.CLIENT_CONTACT_ID = item.CLIENT_CONTACT_ID;
                model.CONTRACTOR_ID = item.CONTRACTOR_ID;
                model.COMPANYID = item.COMPANYID;
                model.CREATOR_ID = item.CREATOR_ID;
                model.UPDATER_ID = item.UPDATER_ID;
                model.TRADECATEGORYID = item.TRADECATEGORYID;
                model.TYPEOFASSESSMENTID = item.TYPEOFASSESSMENTID;
                model.WORK_SITE_ID = item.WORK_SITE_ID;

                model.COVID_SURVEY_ID = item.COVID_SURVEY_ID;
                model.ISCOVIDSURVEY = item.ISCOVIDSURVEY;

                model.WORKORDERNUMBER = item.WORKORDERNUMBER;
                model.PURCHASEORDER = item.PURCHASEORDER;
                model.PROJECTNAME = item.PROJECTNAME;
                model.SCOPEOFWORK = item.SCOPEOFWORK;

                //model.UPDATER_ID = Security.getUserId();
                model.STATUS = 1;
                model.ENTITY_STATUS_ID = 1;
                model.ESCALATION_LEVEL_ID = 1;
                model.WARNING_LEVEL_ID = 1;
                model.STATUS_LEVEL_ID = 1;
                model.WORK_ORDER_ID = null;

                if (model.AssesmentEmployees == null)
                    model.AssesmentEmployees = new List<AssesmentEmployee>();

                foreach (var employee in item.AssesmentEmployees.Where(i => i.IS_ACTIVE == true))
                {
                    model.AssesmentEmployees.Add(new AssesmentEmployee
                    {
                ...
                if (model.AssesmentAttachements == null)
                    model.AssesmentAttachements = new List<AssesmentAttachement>();

                foreach (var attachement in item.AssesmentAttachements)
                {
                    var swms = await context.GetTemplateByTradeId(item.TRADECATEGORYID);

                    //await context.GetTemplateattachments(new Query() { Filter = $@"i => i.TEMPLATEID == {attachement.SWMS_TEMPLATE_ID} && i.TEMPLATETYPEID == {item.TYPEOFASSESSMENTID}" });

                    foreach (var template in swms.Templateattachments)
                    {
                        model.AssesmentAttachements.Add(new AssesmentAttachement
                        {
                            ATTACHEMENTDATE = DateTime.Now,
                            DOCUMENTTEMPLATEURL = template.DOCUMENTURL,
                            SWMS_TEMPLATE_ID = attachement.SWMS_TEMPLATE_ID,
                        });
                    }
                }

                if (model.WORK_ORDER_ID == null)
                {
                    //Create Work Order
                    await CreateWorkOrder(model);
                }
                await ReplaceTextInDocFileForPdf(model);

                //Add Company Transaction History

                var company = await context.GetPersonByPersonId(item.COMPANYID);

                if (company != null && company.Applicence != null)
                {
                    if (model.Transactions == null)
                        model.Transactions = new List<CompanyAccountTransaction>();

                    decimal amount = company.Applicence.NETPRICE * model.AssesmentEmployees.Count();

                    model.Transactions.Add(new CompanyAccountTransaction
                    {
                        ...
                        CURRENCY_ID = (int)company.Applicence.CURRENCY_ID
                    });
                }

                await context.CreateAssesment(model);
            }
            catch (Exception ex)
            {
        ...
        protected async Task<string> ReplaceTextInDocFileForPdf(Assesment refViewModel)
        {
            if (refViewModel != null)
            {
                ...
                foreach (var item in refViewModel.AssesmentAttachements)
                {
                    ...
                    SwmsTemplate swmsTemplate = await context.GetSwmsBySwmsid(item.SWMS_TEMPLATE_ID);


                    Compnay = await context.GetPersonByPersonId(refViewModel.COMPANYID);

                    var refContractor = await context.GetPersonByPersonId(refViewModel.CONTRACTOR_ID);
                    var refContractorContact = await context.GetPersonContactByPersonContactId(refViewModel.CONTRACTOR_CONTACT_ID);
                    var refClient = await context.GetPersonContactByPersonContactId(refViewModel.CONTRACTOR_CONTACT_ID);
                    var refClientContact = await context.GetPersonContactByPersonContactId(refViewModel.CLIENT_CONTACT_ID);
                    ...
                    document.ReplaceText("&&Assessment_Version", swmsTemplate != null ? swmsTemplate.VERSION != null ? swmsTemplate.VERSION : string.Empty : "");
                    ...
                    {
                        var Employee = await context.GetPersonByPersonId(assinedemployee.EMPLOYEE_ID);
                        count++;
                        var nameparagraphs = document.Paragraphs.Where(x => x.Text.Contains("&&Employee" + count.ToString() + "_Name"));
                        if (nameparagraphs != null)
                        {
                            document.ReplaceText("&&Employee" + count.ToString() + "_Name", Employee.FullName);
                        }
                        var singparagraphs = document.Paragraphs.Where(x => x.Text.Contains("&&Emp" + count.ToString() + "_Signature"));
                        if (singparagraphs != null)
                        {
                            document.ReplaceText("&&Emp" + count.ToString() + "_Signature", "&&" + Employee.FIRST_NAME.Trim() + "_" + Employee.LAST_NAME.Trim() + "_" + Employee.PERSON_ID.ToString().Trim());
                        }
                        var dateparagraphs = document.Paragraphs.Where(x => x.Text.Contains("&&Signature" + count.ToString() + "_Date"));
                        if (dateparagraphs != null)
                        {
                            document.ReplaceText("&&Signature" + count.ToString() + "_Date", "&&Sign" + Employee.FIRST_NAME.Trim() + "_" + Employee.LAST_NAME.Trim() + "_date" + "_" + Employee.PERSON_ID.ToString().Trim());
                        }
                    }
                    ...
                    {
                        IList<SwmsTemplatestep> Swmssteps = swmsTemplate.SwmsTemplatesteps.ToList();
                        // swmsTemplate.SwmsTemplatesteps = SwmsTemplatesteps.ToList();
                        //IList<SwmsTemplatestep> Swmssteps = swmsTemplate.SwmsTemplatesteps.ToList();
                        var tableSetupparagraphs = document.Paragraphs.Where(x => x.Text.Contains("&&Steps-table"));
                        var t = document.AddTable(swmsTemplate.SwmsTemplatesteps.Count + 2, 12);
                        ...
                        for (int i = 0; i < Swmssteps.Count; i++)
                        {
                            for (int j = 0; j < 12; j++)
                            {
                                try
                                {
                                    ...
                                    if (j == 8)
                                        t.Rows[i + 2].Cells[j].Paragraphs[0].Append(Swmssteps[i].RiskLikelyhood1 != null ? Swmssteps[i].RiskLikelyhood1.NAME.ToString() : "").FontSize(11);
                                    if (j == 9)
                    ...
                    try
                    {
                        ...
                        {
                            var presults = await context.GetSwmsPperequireds(new Query() { Filter = $@"i => i.SWMSID == {item.SWMS_TEMPLATE_ID}" });
                            IList<SwmsPperequired> SWMSPPERequired = presults.ToList();

                            // swmsTemplate.SwmsPperequireds.ToList();
                    ...
                    try
                    {
                        ...
                        if (plantequipment != null)
                        {
                            ...
                            {
                                var presults = await context.GetSwmsPlantequipments(new Query() { Filter = $@"i => i.SWMSID == {item.SWMS_TEMPLATE_ID}" });
                                swmsTemplate.SwmsPlantequipments = presults.ToList();
                            }

                            if (swmsTemplate.SwmsPlantequipments != null)
                            {
                                foreach (var plnt in swmsTemplate.SwmsPlantequipments)
                                {
                    ...
                    try
                    {
                        ...
                        if (hazardmaterialspara != null)
                        {
                            ...
                            {
                                var presults = await context.GetSwmsHazardousmaterials(new Query() { Filter = $@"i => i.SWMSID == {item.SWMS_TEMPLATE_ID}" });
                                swmsTemplate.SwmsHazardousmaterials = presults.ToList();
                            }
                            ...
                            if (swmsTemplate.SwmsHazardousmaterials != null)
                            {
                                ...
                                foreach (var plnt in swmsTemplate.SwmsHazardousmaterials)
                                {
                    ...
                    try
                    {
                        ...
                        if (referencedlegislationpara != null)
                        {
                            ...
                            {
                                var presults = await context.GetSwmsReferencedlegislations(new Query() { Filter = $@"i => i.SWMSID == {item.SWMS_TEMPLATE_ID}" });
                                swmsTemplate.SwmsReferencedlegislations = presults.ToList();
                            }

                            if (swmsTemplate.SwmsReferencedlegislations != null)
                            {
                                foreach (var plnt in swmsTemplate.SwmsReferencedlegislations)
                                {
                    ...
                    try
                    {
                        ...
                        if (licensesandpermitspara != null)
                        {
                            ...
                            {
                                var presults = await context.GetSwmsLicencespermits(new Query() { Filter = $@"i => i.SWMSID == {item.SWMS_TEMPLATE_ID}" });
                                swmsTemplate.SwmsLicencespermits = presults.ToList();
                            }

                            if (swmsTemplate.SwmsLicencespermits != null)
                            {
                                foreach (var plnt in swmsTemplate.SwmsLicencespermits)
                                {
                    ...
                    try
                    {
                        ...
                        string docPath = Path.Combine(filePath, filePath + "/Uploads/Templates/Doc") + $@"\{swmsTemplate.TEMPLATENAME + "-" + item.DOCUMENTTEMPLATEURL}";
                        document.SaveAs(docPath);
                        item.DOCUMENTTEMPLATEURL = swmsTemplate.TEMPLATENAME + "-" + item.DOCUMENTTEMPLATEURL;
        ...
        protected async Task CreateWorkOrder(Assesment assesment)
        {
            ...
            //int maxID = await OrderService.GetMaxID(assesment.COMPANYID);          
            ...
            assesment.WorkOrder = new WorkOrder()
            {
                ...
                CREATED_DATE = DateTime.Now,
                UPDATED_DATE = DateTime.Now,
                CREATOR_ID = assesment.CREATOR_ID,
                UPDATER_ID = assesment.CREATOR_ID,
                IS_DELETED = false,
                WARNING_LEVEL_ID = 1,
                ESCALATION_LEVEL_ID = 1,
                DUE_DATE = assesment.ASSESMENTDATE,
                //WORK_ORDER_NUMBER = GetNextValue(current, maxID),
                WORK_ORDER_NUMBER = GetNextValue(assesment.WORKORDERNUMBER),
            ...
            //Assigned Worker
            foreach (var employee in assesment.AssesmentEmployees)
            {
        ...
        private string GetNextValue(string str)
        {
            ...
            var currentDateFormat = time.ToString("ddMMyyyy-HHmm");
            return result = str + "-" + currentDateFormat;
        }
        ...
        private string GetFormNumber(string str)
        {
            ...
            DateTime time = DateTime.Now;
            var currentDateFormat = time.ToString("ddMMyyyy-HHmm");
            result = str + "-" + currentDateFormat;
            return result;
        }


        public async Task SendInsufficientBalanceEmail(Person args)
        {
            Smtpsetup smtpsetup = await context.GetSmtpsetup();
            if (smtpsetup != null)
            {
                ...
                mimeMessage.Subject = "Insufficient Balance Notification";
                string emailbody = "You have insufficient balance so no more assessments will be generated, please add funds into your account to continue";
