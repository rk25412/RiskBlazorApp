...
namespace Clear.Risk.Infrastructures.Helpers
{
    public static class IdentityExtensions
    {
        ...
        {
            var claim = ((ClaimsIdentity)identity).Claims.Where(c => c.Type == "CompanyId").Select(c => c.Value).SingleOrDefault();
            return (claim != null) ? int.Parse(claim) : -1;
        }

        public static int GetCountryId(this IIdentity identity)
        {
            var claim = ((ClaimsIdentity)identity).Claims.Where(c => c.Type == "CountryId").Select(c => c.Value).SingleOrDefault();
            return (claim != null) ? int.Parse(claim) : -1;
        }
        ...
        public static int? GetUserBusinessUnitId(this IIdentity identity)
        {
            var claim = ((ClaimsIdentity)identity).Claims.Where(c => c.Type == "BusinessUnitId").Select(c => c.Value).SingleOrDefault();
            return (!string.IsNullOrEmpty(claim)) ? int.Parse(claim) : (int?)null;
        }

        public static int? GetUserDivisionId(this IIdentity identity)
        {
            var claim = ((ClaimsIdentity)identity).Claims.Where(c => c.Type == "DivisionId").Select(c => c.Value).SingleOrDefault();
            return (!string.IsNullOrEmpty(claim)) ? int.Parse(claim) : (int?)null;
        }

        public static int? GetUserSuperVisorId(this IIdentity identity)
        {
            var claim = ((ClaimsIdentity)identity).Claims.Where(c => c.Type == "SUPERVISOR_ID").Select(c => c.Value).SingleOrDefault();
            return (!string.IsNullOrEmpty(claim)) ? int.Parse(claim) : (int?)null;
        }
    }
}
